...
#replicate the STATA variables the researchers made
...
#chatgpt helped create line below I need to remove C() to get actual column name
control_vars_list=[apple if 'C(' not in apple else apple.split('C(')[1].split(')')[0] for apple in control_vars_list]
#chatgpt helped create line above I need to remove C() to get actual column name
...
print("HERE")
print(soln_path)
soln_path.to_csv("ridge_soln_path", index=True)
...
ax.set_ylim([mean_test_score.min() * 0.9, mean_test_score.max() * 1.1])
ax.set_xlabel('$-\log(\lambda)$', fontsize=20)
...
control_vars_list=[apple if 'C(' not in apple else apple.split('C(')[1].split(')')[0] for apple in control_vars_list]
#chatgpt helped create line above I need to remove C() to get actual column name (same line from above)
...
print("HERE")
print(soln_path)
soln_path.to_csv("lasso_soln_path", index=True)
...
hist_imp = feature_imp.sort_values(by='importance', ascending=False)
plt.figure(figsize=(12,8))
plt.barh(hist_imp.index, hist_imp['importance'])
plt.savefig("Hist_IMP", bbox_inches='tight')
...
key_variables = ['female', 'm_origin', 'dad_midedu', 'm_dad_edu', 'm_mom_edu', 'dad_lowedu', 'dad_highedu', 'mom_employed', 'enrol_sch_snv', 'immigrants_broad', 'answers_math_pct', 'answers_ital_pct', 'clsize_snv', 'our_CHEAT_math', 'our_CHEAT_ital', 'answers_math_std', 'answers_ital_std', 'clsize_hat', 'o_math', 'interaction_effect_eg_clsize','edu_gap']
...
fig, place = plt.subplots(4, 2, figsize=(14,18))

place[0, 0].hist(data['clsize_snv'].dropna(), edgecolor='black')
place[0, 0].set_title('(a) Class Size')

place[0, 1].hist(data['answers_math_pct'].dropna(), edgecolor='black')
place[0, 1].set_title('(b) Math Grades')


place[1, 0].hist(data['answers_ital_pct'].dropna(), edgecolor='black')
place[1, 0].set_title('(c) Italian Grades')

place[1, 1].hist(data['answers_math_std'].dropna(), edgecolor='black')
place[1, 1].set_title('(d) Standardized Math Grades')


place[2, 0].hist(data['answers_ital_std'].dropna(), edgecolor='black')
place[2, 0].set_title('(e) Standardized Italian Grades')


place[2, 1].hist(data['edu_gap'].dropna(), edgecolor='black')
place[2, 1].set_title('(f) Parent Education Gap')


place[3, 0].hist(data['enrol_sch_snv'].dropna(), edgecolor='black')
place[3, 0].set_title('(g) School Enrollment')

place[3, 1].hist(data['interaction_effect_eg_clsize'].dropna(), edgecolor='black')
place[3, 1].set_title('(h) Parent Education Gap * Class Size')
